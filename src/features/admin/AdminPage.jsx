...
export default function AdminPage() {
  ...
  return (
    <div className="relative flex gap-8 container mx-auto font-Quicksand selection:bg-Cultured mb-6">
      <button onClick={() => setMenuOpen(true)} className="lg:hidden absolute right-10 top-5 cursor-pointer w-9 h-9 flex items-center justify-center p-4
     bg-gray-200 text-gray-700 rounded-full m-4 z-50">
      ...
      <main className="flex-1 p-4 sm:p-8">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between mb-6">
          ...
          <button onClick={() => setModalOpen(true)} className="px-4 py-2 bg-gray-200 text-gray-600 font-semibold rounded-xl shadow-md hover:shadow-gray-500/50
        ...
        {editModalOpen && editProduct && (
          <EditModal closeModal={() => setEditModalOpen(false)} product={editProduct}/>
        )}

        {deleteModal && (
          <DeleteModal closeModal={() => setDeleteModal(false)} />
        )}
                ...
                products.map((product) => (
                  <tr>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="inline-block px-3 py-1 bg-gray-100 text-gray-700 font-bold rounded-full text-xs sm:text-sm">
                        ${product.price}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => { setEditProduct(product); setEditModalOpen(true);}} className="group relative p-2 sm:p-3 bg-Cultured cursor-pointer text-indigo-600 
                        rounded-xl hover:shadow-md duration-500 hover:bg-indigo-50 transition-colors">
                          <i className="fa-solid fa-pen-to-square text-sm"></i>
                          <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                            Edit
                          </span>
                        </button>
                        <button onClick={() => setDeleteModal(true)} className="group relative p-2 sm:p-3 bg-Cultured cursor-pointer text-red-500 
                        rounded-xl hover:shadow-md duration-500 hover:bg-red-50 transition-colors">
                          <i className="fa-solid fa-trash text-sm"></i>
                          <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-500 whitespace-nowrap">
                            Delete
                          </span>
                        </button>
                      </div>
